...
// âncoras para o carrossel de destaques
...
const Products = () => {
    ...
    useEffect(() => {
        ...
        const onSelect = () => {
            try {
                setHighlightIndex(autoCarouselApi.selectedScrollSnap());
            } catch { }
        };
        onSelect();
    ...
    return (
        <section ref={sectionRef} id="produtos" className="w-full py-20 bg-brand-yellow texture-dots-light">
            <div className="container mx-auto px-6">
                ...
                {/* Carrossel de destaques (sem mudanças estruturais) */}
                ...
                {/* Seções de produto: layout padronizado ao estilo “sorvetes” */}
                <div className="mt-20 space-y-20">
                    {productLines.map((line) => {
                        const sharedImage = isSupportedImageUrl(line.sharedImage) ? line.sharedImage : undefined;

                        // usa layout com imagem fixa se: há sharedImage E (todos os slides não têm imagem OU usam exatamente a sharedImage)
                                    ...
                                    <div className="flex flex-col gap-10 p-8 sm:p-12">
                                        <div className="flex flex-col gap-3 text-brand-dark text-center">
                                            <h4 className="text-3xl sm:text-4xl font-playfair font-bold text-brand-purple">
                                                {(line.title || "").replace(/^Linha\s+/i, "").trim()}
                                            </h4>
                                        ...
                                        {/* ====== LAYOUT PADRÃO ====== */}
                                        {usesSharedImage ? (
                                            // 1) Imagem fixa à esquerda + carrossel de textos/itens à direita
                                                                        ...
                                                                        <ul className="mt-4 space-y-3 w-full max-w-xl self-center">
                                                                            {slide.items.map((it) => (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <span className="flex-1 text-sm md:text-base">{it}</span>
                                                                                </li>
                                                                            ))}
                                                                        </ul>
                                                                    )}
                                            ...
                                            // 2) Imagem por slide à esquerda + textos/itens do slide à direita
                                                        ...
                                                        return (
                                                            <CarouselItem key={slide.id} className="pl-6 basis-full">
                                                                <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:gap-12">
                                                                    ...
                                                                    {/* Direita: conteúdo com o mesmo “cartão roxo” do Sorvetes */}
